...
class _RegionPair:
    ...
    @property
    def modulation_lfo_to_pitch(self) -> int:
        return self._get_value(_GeneratorType.MODULATION_LFO_TO_PITCH)

    @property
    def vibrato_lfo_to_pitch(self) -> int:
        return self._get_value(_GeneratorType.VIBRATO_LFO_TO_PITCH)

    @property
    def modulation_envelope_to_pitch(self) -> int:
        return self._get_value(_GeneratorType.MODULATION_ENVELOPE_TO_PITCH)

    @property
    def initial_filter_cutoff_frequency(self) -> float:
        return _SoundFontMath.cents_to_hertz(self._get_value(_GeneratorType.INITIAL_FILTER_CUTOFF_FREQUENCY))

    @property
    def initial_filter_q(self) -> float:
        return 0.1 * self._get_value(_GeneratorType.INITIAL_FILTER_Q)

    @property
    def modulation_lfo_to_filter_cutoff_frequency(self) -> int:
        return self._get_value(_GeneratorType.MODULATION_LFO_TO_FILTER_CUTOFF_FREQUENCY)

    @property
    def modulation_envelope_to_filter_cutoff_frequency(self) -> int:
        return self._get_value(_GeneratorType.MODULATION_ENVELOPE_TO_FILTER_CUTOFF_FREQUENCY)

    @property
    def modulation_lfo_to_volume(self) -> float:
        return 0.1 * self._get_value(_GeneratorType.MODULATION_LFO_TO_VOLUME)

    @property
    def chorus_effects_send(self) -> float:
        return 0.1 * self._get_value(_GeneratorType.CHORUS_EFFECTS_SEND)

    @property
    def reverb_effects_send(self) -> float:
        return 0.1 * self._get_value(_GeneratorType.REVERB_EFFECTS_SEND)

    @property
    def pan(self) -> float:
        return 0.1 * self._get_value(_GeneratorType.PAN)

    @property
    def delay_modulation_lfo(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.DELAY_MODULATION_LFO))

    @property
    def frequency_modulation_lfo(self) -> float:
        return _SoundFontMath.cents_to_hertz(self._get_value(_GeneratorType.FREQUENCY_MODULATION_LFO))

    @property
    def delay_vibrato_lfo(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.DELAY_VIBRATO_LFO))

    @property
    def frequency_vibrato_lfo(self) -> float:
        return _SoundFontMath.cents_to_hertz(self._get_value(_GeneratorType.FREQUENCY_VIBRATO_LFO))

    @property
    def delay_modulation_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.DELAY_MODULATION_ENVELOPE))

    @property
    def attack_modulation_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.ATTACK_MODULATION_ENVELOPE))

    @property
    def hold_modulation_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.HOLD_MODULATION_ENVELOPE))

    @property
    def decay_modulation_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.DECAY_MODULATION_ENVELOPE))

    @property
    def sustain_modulation_envelope(self) -> float:
        return 0.1 * self._get_value(_GeneratorType.SUSTAIN_MODULATION_ENVELOPE)

    @property
    def release_modulation_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.RELEASE_MODULATION_ENVELOPE))

    @property
    def key_number_to_modulation_envelope_hold(self) -> int:
        return self._get_value(_GeneratorType.KEY_NUMBER_TO_MODULATION_ENVELOPE_HOLD)

    @property
    def key_number_to_modulation_envelope_decay(self) -> int:
        return self._get_value(_GeneratorType.KEY_NUMBER_TO_MODULATION_ENVELOPE_DECAY)

    @property
    def delay_volume_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.DELAY_VOLUME_ENVELOPE))

    @property
    def attack_volume_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.ATTACK_VOLUME_ENVELOPE))

    @property
    def hold_volume_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.HOLD_VOLUME_ENVELOPE))

    @property
    def decay_volume_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.DECAY_VOLUME_ENVELOPE))

    @property
    def sustain_volume_envelope(self) -> float:
        return 0.1 * self._get_value(_GeneratorType.SUSTAIN_VOLUME_ENVELOPE)

    @property
    def release_volume_envelope(self) -> float:
        return _SoundFontMath.timecents_to_seconds(self._get_value(_GeneratorType.RELEASE_VOLUME_ENVELOPE))

    @property
    def key_number_to_volume_envelope_hold(self) -> int:
        return self._get_value(_GeneratorType.KEY_NUMBER_TO_VOLUME_ENVELOPE_HOLD)

    @property
    def key_number_to_volume_envelope_decay(self) -> int:
        return self._get_value(_GeneratorType.KEY_NUMBER_TO_VOLUME_ENVELOPE_DECAY)

    @property
    def initial_attenuation(self) -> float:
        return 0.1 * self._get_value(_GeneratorType.INITIAL_ATTENUATION)

    @property
    def coarse_tune(self) -> int:
        return self._get_value(_GeneratorType.COARSE_TUNE)

    @property
    def fine_tune(self) -> int:
        return self._get_value(_GeneratorType.FINE_TUNE) + self._instrument.sample.pitch_correction
    ...
    @property
    def scale_tuning(self) -> int:
        return self._get_value(_GeneratorType.SCALE_TUNING)
